...
from metrics import evaluate_solution_metrics
...
CHORD_INTERVALS_EXPORT = {
    ...
    "dim7":     ["P1", "m3", "d5", "d7"],
    "it6":      ["P1", "M3", "A6"],  # ♭6–1–♯4 if root is ♭6
}
...
def main(pkl_path: str) -> None:
    ...
    melody = Melody(Path(melody_path))
    ...
    if metrics_rows:
        ...
        if len(dfm) > 1:
            ...
            print(f"  CTnCTR mean±std = {mean['CTnCTR']:.6f} ± {std['CTnCTR']:.6f}")
            print(f"  PCS    mean±std = {mean['PCS']:.6f} ± {std['PCS']:.6f}")
            print(f"  MCTD   mean±std = {mean['MCTD']:.6f} ± {std['MCTD']:.6f}")
