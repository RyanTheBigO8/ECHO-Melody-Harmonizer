...
from chord_pool.pool_policy import list_policy_groups
from chord_pool.build_pool import prepare_harmonization
from ga import HarmonizeGA

from fitness.fitness_tonal import FitnessEvaluatorTonal
from fitness.fitness_atonal import FitnessEvaluatorAtonal
from fitness.fitness_yeh import FitnessEvaluatorYeh

from chord_pool.chords_master import MASTER_CHORD_TABLE
from chord_pool.chord_dist import load_or_build_distance_matrix, load_or_build_tonal_distance_matrix
...
def main() -> None:
    ...
    style = parse_style_arg("auto" if args.auto else args.style)
    density = parse_density_arg("auto" if args.auto else args.density)
    ...
    melody_stem = Path(args.melody_path).stem
    out_dir = Path("./../output") / melody_stem
    if out_dir.exists():
        os.chmod(out_dir, 0o755) # 0o755 means rwxr-xr-x
    ...
    base_run_kwargs = dict(
        ...
        save_best_every=run_cfg.save_best_every,
        save_top_frac=run_cfg.save_top_frac,
        save_top_every=run_cfg.save_top_every,
