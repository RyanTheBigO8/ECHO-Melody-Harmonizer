...
from echo_harmonizer.fitness.fitness_protocol import FitnessEvaluatorProtocol
...
# ============================================================
# GA driver class (FULL re-evaluation on change)
# ============================================================

class HarmonizeGA:
    """
    Full-reevaluation GA (ablation baseline).

    Difference vs ga.py:
      - No partial / incremental updates.
      - If a child changed (crossover and/or mutation), recompute *all* region fitness values
        + global fitness for that individual.
    """
    ...
    def __init__(
        ...
        *,
        ...
        print(f"[GA_SLOW] Using {self.n_threads} thread(s) for FULL fitness recomputation.")
    ...
    def _recompute_full_inplace(self, ind: Individual) -> None:
        idxs = list(range(self.H))
        pairs = self._compute_region_fitness_many(ind.genes, idxs)
        ...
        g = self._compute_global_fitness(ind.genes)

        ind.region_fitness = rf
        ind.global_fitness = float(g)
        ind.total_fitness = float(region_total + g)
    ...
    # ----------------------------
    # Crossover + Mutation (track change)
    # ----------------------------

    def crossover_single_point(self, p1: Individual, p2: Individual, pc: float) -> tuple[Individual, Individual, bool]:
        ...
        # region_fitness copied (will be recomputed fully if changed=True)
        c1_rf = p1.region_fitness[:]  # placeholder
        c2_rf = p2.region_fitness[:]  # placeholder

        c1 = Individual(c1_genes, c1_rf, p1.global_fitness, p1.total_fitness)
        c2 = Individual(c2_genes, c2_rf, p2.global_fitness, p2.total_fitness)
        return c1, c2, True

    def mutate_uniform_reset(self, child: Individual, pm_gene: float) -> bool:
        changed = False
    ...
    def run(
        ...
        *,
        ...
        if use_tqdm and tqdm is not None:
            it = tqdm(it, desc="GA_SLOW", total=int(max_gens), dynamic_ncols=True)
        ...
        try:
            for gen in it:
                ...
                if use_tqdm and tqdm is not None:
                    ...
                    print(f"[GA_SLOW] gen={gen:4d}  avg={avg_fit:.3f}  best={best.total_fitness:.3f}  pc={pc:.2f}  pm={pm:.2f}  stagnant={stagnant}/{stagnation_gens}")
                    ...
                    if best_avg is None or avg_fit >= best_avg * (1.0 + float(stagnation_improve)):
                        ...
                        if stagnant >= int(stagnation_gens):
                            ...
                            if Fore is not None and Style is not None:
                                print("[GA_SLOW] " + Fore.BLUE + msg + Style.RESET_ALL)
                            else:
                                print("[GA_SLOW] " + msg)
                            break
                ...
                for p1, p2 in pairs:
                    c1, c2, did_xo = self.crossover_single_point(p1, p2, pc)

                    did_mut1 = self.mutate_uniform_reset(c1, pm)
                    did_mut2 = self.mutate_uniform_reset(c2, pm)

                    # FULL reevaluation whenever changed
                    if did_xo or did_mut1:
                        self._recompute_full_inplace(c1)
                    # else: keep copied fitness from parent

                    if did_xo or did_mut2:
                        self._recompute_full_inplace(c2)
